...
学生将获取从课程和案例研究中获得的信息，以确定 AdventureWorks 中数字转换项目的可交付成果。通过检查 Azure 门户以及其他工具，学生将确定如何利用本机工具来识别和解决与性能相关的问题。最后，学生将能够识别数据库中的碎片，并学习恰当解决问题的步骤。 
...
1. 导航回 portal.azure.com。你可能需要再次登录。在 Azure 门户顶部的搜索栏中，键入“SQL”，然后单击 SQL 数据库。该任务将使用你在实验3中创建的 AdventureWorks 数据库。单击数据库名称。


    ![社交媒体帖子屏幕截图 描述自动生成](../images/dp-3300-module-44-lab-01.png)


2. 在 AdventureWorks 数据库的边栏选项卡的“监视”部分找到指标图标。 

    ![图片 4](../images/dp-3300-module-44-lab-02.png)
...
1. 在第一个任务所示的同一视图中，在同一菜单中选择“Query Performance Insight”
...
1.  在实验室 VM 中，打开开始菜单并导航到“Microsoft SQL Server Tools 18”>“Microsoft SQL Server Management Studio”，打开 SQL Server Management Studio。 
...
5. 单击“新建查询”按钮以打开另一个查询窗口。将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮以执行此查询。 
...
6. 在对象资源管理器中，导航到“管理”，然后通过单击加号来展开配置单元。展开“扩展事件”配置单元，然后展开“会话”配置单元。展开在步骤 3 中创建的“阻止”会话。右键单击 package0.ring_buffer 并选择查看目标数据。 
...
9.  右键单击扩展事件会话，然后选择“停止会话”。 

10.  导航回到你在步骤 4 中打开的“查询”选项卡，并在查询的下一行键入“ROLLBACK TRANSACTION”。突出显示 ROLLBACK TRANSACTION 并单击执行该命令。

    ![图片1](../images/dp-3300-module-44-lab-13.png)

11. 导航回到在步骤 5 中打开的查询标签。你会注意到查询已完成。

## 任务 2：启用读取提交快照隔离

1. 将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮以执行此查询。 
...
GO
```
2.  开始对 AdventureWorks2017 数据库的新查询。将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮，或者按 t 键执行此查询。 
```sql
USE AdventureWorks2017
...
3. 针对 AdventureWorks2017 数据库启动另一个新查询。将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮以执行此查询。 
...
4. 与讲师讨论为什么步骤 3 中的查询现已完成，而在上一个练习中，它被 UPDATE 所阻止。 
...
1. 选择其中一个要求，并确定要执行哪些高级任务来满足所选的数据要求。

2. 教师将与小组讨论结果。

## 任务 1：识别碎片

1. 在实验室 VM 上启动 SQL Server Management Studio。输入服务器名称   
DP-300-SQL1，确保选定“Windows 身份验证”，然后单击“连接”。单击“新建查询”按钮。将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮以执行此查询。 
...
2. 将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮以执行此查询。 
...
3. 将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮执行此查询。 
...
1. 单击“新建查询”按钮。将以下 T-SQL 代码复制并粘贴到查询窗口中。单击“执行”按钮以执行此查询。 
...
3. 重新执行任务 1 中步骤 3 中的查询。确认 AK_Address_StateProvinceID 索引不再具有大于 50％ 的碎片。 

4. 重新执行任务 1 中步骤 4 中的查询。记录 Management Studio “结果”窗格的“消息”选项卡中的逻辑读取次数。重新生成索引之前遇到的逻辑读取次数是否有变化？ 
