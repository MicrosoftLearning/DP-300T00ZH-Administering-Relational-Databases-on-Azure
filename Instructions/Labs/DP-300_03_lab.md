...
9. 在数据库设置屏幕上，将 Azure SQL 数据库的版本更改为“常规用途”。将服务目标更改为 GP_Gen4_2，然后单击“下一步”。 
...
12. 通过将文本粘贴到查询窗口中并单击“执行”或按 F5 键来执行以下 T-SQL 查询。
	...
	将 192.168.1.1. 替换为步骤 4 中的客户端 IP 地址。接下来，将在 AdventureWorks 数据库中创建一个包含用户。单击“新建查询”，然后执行以下 T-SQL。确保正在使用 AdventureWorks 数据库。如果在下面突出显示的框中看到 master，则可以下拉并切换到 AdventureWorks。
...
3. 打开 SQL Server Management Studio，然后单击“连接”>“数据库引擎”。在“服务器名称”中，输入你的服务器名称。将身份验证类型更改为“Azure Active Directory 通用且具有 MFA 支持”。
	...
	按提示输入 Azure Active Directory 密码，然后登录数据库。 
