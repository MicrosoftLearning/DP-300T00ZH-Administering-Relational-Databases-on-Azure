...
学生将利用从课程和案例研究中获取的信息，在 Azure 门户和 AdventureWorks 数据库中进行配置，随后实现安全性。 
...
1. 使用 Azure 门户配置基于服务器的防火墙规则

2. 部署数据库

3. 使用 T-SQL 添加数据库防火墙规则

4. 使用特定于数据库的连接字符串连接到该数据库

## 任务 1：    使用 Azure 门户配置基于服务器的防火墙规则
...
4. 单击在复制到剪贴板时单击的服务器名称上方的“显示防火墙设置”。单击下面突出显示的“+ 客户端 IP”，然后单击“保存”。

 

	![图片 3](../images/dp-3300-module-33-lab-04.png)

	这将你能够使用 SQL Server Management Studio 或任何其他客户端工具连接到 Azure SQL 数据库服务器。记下你的客户端 IP 地址，稍后将在此任务中使用它。
...
9. 在数据库设置屏幕上，将 Azure SQL 数据库的版本更改为“常规用途”。将“服务目标”更改为 **GP_Gen5_2**，然后单击“下一步”。[请注意，屏幕截图中所示的 **GP_Gen4_2** 在 2020 年 8 月后将无法使用。]
...
12. 将文本粘贴到查询窗口中并将 192.168.1.1. 替换为步骤 4 中的客户端 IP 地址，以执行以下 T-SQL 查询。单击“执行”或按 F5。
	...
	接下来，将在 AdventureWorks 数据库中创建一个包含用户。单击“新建查询”，然后执行以下 T-SQL。确保仍在使用 AdventureWorks 数据库。如果在下面突出显示的框中看到 master，则可以下拉并切换到 AdventureWorks。


	![手机的屏幕截图；描述自动生成](../images/dp-3300-module-33-lab-13.png)

13. 单击“执行”运行此命令。此命令会在 AdventureWorks 数据库中创建一个包含的用户。将在下一步中使用此用户名和密码登录。
...
15. 。尝试使用步骤 13 创建的凭据建立连接。将看到以下错误。


	![手机的屏幕截图；描述自动生成](../images/dp-3300-module-33-lab-15.png)

	这是因为该连接尝试登录 master 数据库，而不是创建用户的 AdventureWorks。若要更改连接上下文，请单击“确定”退出错误消息，然后单击“连接到服务器”对话框中的选项，如下所示。
...
## 任务 2：使用 Azure Active Directory 授权对 Azure SQL 数据库的访问
...
1. 导航到 Azure 门户，然后在屏幕右上角单击你的用户名。
	...
	**注意：** Azure SQL 数据库的 Azure Active Directory 管理员不支持 Microsoft 帐户（例如 Outlook、Gmail、Hotmail 或 Yahoo 的用户帐户）。你可以使用一种变通方法，即创建一个名为 DBA 的 Azure Active Directory 组，然后将你的用户帐户添加到其中。或者，你可以跳过任务 2。
...
1. 从 Azure SQL 服务器的主边栏选项卡中，导航到“安全”部分，然后单击“高级数据安全”。 
 

	![图片 14](../images/dp-3300-module-33-lab-24.png)

	
	单击“高级数据安全”下的切换开关，切换到“开”。单击“存储帐户”。


	![图片 1032870623](../images/dp-3300-module-33-lab-25.png)

	

	系统将提示创建存储帐户。选择在实验室 1 中创建的存储帐户。将执行定期扫描开关切换到“开”。选择一个用于接收扫描报告和警报的电子邮件帐户。取消选中“也向管理员和订阅所有者发送电子邮件通知”复选框。
 


	![社交媒体帖子屏幕截图 描述自动生成](../images/dp-3300-module-33-lab-26.png)

	单击“高级威胁检测”类型，然后查看各选择。保持选中所有框，然后单击屏幕顶部的 **“保存”**。


2. 在 Azure SQL 服务器的概述屏幕中向下滚动并单击数据库名称，从而导航到 Azure 门户中的 AdventureWorks 数据库。 
...
3. 导航到 Azure SQL 数据库主边栏选项卡的“安全”部分，然后单击“高级数据安全”。在“高级数据安全”屏幕中，单击“数据发现和分类”。


	![图片 18](../images/dp-3300-module-33-lab-28.png)

	下一个屏幕会显示一条信息性消息，内容为“已找到 32 个附带分类建议的列”。单击该链接。
...
4. 返回到你的 VM 上的 SQL Server Management Studio。从 AdventureWorks 数据库启动新的查询窗口，然后执行以下查询。
	...
	![手机的屏幕截图；描述自动生成](../images/dp-3300-module-33-lab-31.png)

 

## 任务 4：管理对数据库对象的访问

1. 在此任务中，你将管理对数据库及其对象的访问。首先要做的是在 AdventureWorks 数据库中创建两个用户。打开一个新的查询窗口，然后将下面的 T-SQL 复制并粘贴到其中。执行查询以创建两个用户。
...
5. 接下来，将使用 EXECUTE AS USER 语法测试刚刚建立的安全性。这使数据库引擎可以在你的用户上下文中执行查询。在查询窗口中执行以下查询。
