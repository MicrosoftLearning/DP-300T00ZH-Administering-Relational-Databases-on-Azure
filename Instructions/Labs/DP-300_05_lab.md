...
**先决条件**: 学生还将登录到运行 SQL Server 的 VM。
...
1. 从实验室虚拟机中，启动 **SQL Server Management Studio (SSMS)**。在 Management Studio 中单击“新建查询”按钮，启动新查询。

	![手机的屏幕截图；描述自动生成](../images/dp-3300-module-55-lab-01.png)

2. 系统将提示你连接到 SQL Server。  
输入服务器名称 localhost，并确保选中“ Windows 身份验证”，然后单击“连接”。

	![图片 1570495882](../images/dp-3300-module-55-lab-02.png)

‎	
3. 单击 SQL Server Management Studio 中的“新建查询”按钮启动新查询。将以下代码复制并粘贴到查询窗口中。
...
6. 通过在 SSMS 的结果面板中单击执行计划选项卡，导航到执行计划。在执行计划中，使鼠标悬停在 SELECT 运算符上方。你将注意到由黄色三角形中的感叹号标识的警告消息，如下所示。确定警告消息告诉你的内容。 
...
如果查看任务 1 中的查询，你将注意到与 WHERE 子句中的 NationalIDNumber 列比较的值以数字形式传递，因为它不在带引号的字符串中。检查表结构后，你将发现该表中的此列使用 nvarchar(15) 数据类型，而不是 int 或 integer 数据类型。这种数据类型的不一致导致优化器在执行时将常量隐式转换为 nvarchar，从而导致使用次优计划的查询性能产生额外的开销。

2. 更改代码以解决隐式转换，然后重新运行查询。如果上面的练习尚未开启“包括实际执行计划” (Cnt l+ M)，请记住将其开启。请注意，警告现在已经消失。

更改 WHERE 子句，以使与 NationalIDNumber 列比较的值和表中该列的数据类型一致，这样就可以消除隐式转换。在这种情况下，只需在值的两边加上单引号，就可以将其从数字更改为字符格式。使此查询的查询窗口保持打开状态。
...
要尝试修复索引，请在新的查询窗口中，复制并粘贴以下查询以更改此列的数据类型。通过单击“执行”或按下 F5，尝试执行查询。
...
有几种方法可以在 SQL Server Management Studio 中生成执行计划。你将使用练习 1 中的同一种查询。将下面的代码复制并粘贴到新的查询窗口中，然后通过单击“执行”或按 F5 键来执行该代码。
...
1. 将下面代码复制粘贴到新的查询窗口中。在运行查询之前，单击下文所示的包括实际执行计划图标，或按“Ctrl+M”。通过单击“执行”或按“F5”键来执行查询。记下消息选项卡中的执行计划和逻辑读取。
...
在查看执行计划时，你将注意到存在键查找。如果将鼠标悬停在该图标上，将看到属性指示对查询检索到的每一行执行该操作。可以看到执行计划正在执行键查找操作。 

要确定需要更改哪个索引来删除键查找，需要检查其上方的索引搜寻。将鼠标悬停在索引查找运算符上，此时将显示该运算符的属性。记下输出列，如下所示。 
...
2. 修复键查找，并重新运行查询以查看新计划。通过添加包含所有查询返回或搜索列的 COVERING 索引来修复键查找。在本示例中，该索引只有**ProductID**。如果我们将输出列表添加到该索引作为包含的列，并添加其他搜索列（**ModifiedDate**）作为键列，则会移除键查找。由于已存在该索引，因此必须删除索引并重新创建索引，或者设置 DROP_EXISTING = ON 方可添加列。备注：**ProductID**已经是索引的一部分，无需添加为包含的列。
...
2. 在 SQL Server Management Studio 中，单击“文件”>“打开”>“文件控制”。导航到“D:\Labfiles\Query Performance\CreateRandomWorkloadGenerator.sql”文件。单击文件将其加载到 Management Studio 中，然后单击“执行”或按“F5”键执行查询。

![社交媒体帖子屏幕截图 描述自动生成](../images/dp-3300-module-55-lab-09.png)

 
3. 运行工作负载以生成查询存储的统计信息。导航回“D:\Labfiles\Query Performance\ExecuteRandomWorkload.sql”脚本以执行工作负载。单击“执行”或按“F5”运行此脚本。执行完成后，再次运行脚本。使此查询的查询选项卡保持打开状态。

4. 将下面代码复制并粘贴到新的查询窗口中，然后单击“执行”或按“F5”执行查询。此脚本使用以下脚本将数据库兼容性模式更改为 SQL Server 2019 (150)
...
5. 从步骤 3 导航回到查询选项卡，然后重新执行。

## 任务 2：检查最消耗资源的查询以确定较差性能

1. 为了查看查询存储，需要在 Management Studio 中刷新 AdventureWorks2017 数据库。右键单击数据库名称，然后选择单击刷新。然后你将在数据库下看到“查询存储”选项。  
‎
	![手机的屏幕截图；描述自动生成](../images/dp-3300-module-55-lab-10.png)  
‎

2. 展开查询存储节点以查看所有可用报告。单击“正号”以展开查询存储报告。选择“最消耗资源的查询”报告
...
2. 单击报告右上方窗口中持续时间最短的计划 ID（由图表 Y 轴下部位置指示）。此计划 ID 是上图中的 PlanID 43。单击“计划摘要”图表旁边的计划 ID（应突出显示，如上面的屏幕截图所示）。
...
‎
	...
	- 现在运行下一个查询。只是这次将 SalesPersonID 值更改为 277。请注意执行计划中的“聚集索引扫描”操作。
        ```sql
		USE [AdventureWorks2017]
	...
	由于此 where 子句的值不同，数据库优化器基于列统计信息选择了不同的执行计划。由于此查询在其 WHERE 子句中使用常量，因此优化程序将把这些查询视为唯一查询，并为每个查询生成不同的执行计划。

## 任务 2：使用参数和查询提示更改查询

1. 对 SalesPersonID 使用变量值来更改查询

2. 使用 T-SQL DECLARE 语句声明@SalesPersonID，以便可以传递值，而不是在 WHERE 子句中硬编码该值。应该确保变量的数据类型与目标表中列的数据类型匹配。 
...
再次执行此查询，将参数值更改为 288。如果检查执行计划，你会注意到它与 277 的值相同。这是因为 SQL Server 已缓存了执行计划，并且正重用于查询的第二次执行。请注意，尽管两个查询使用相同的计划，但不一定是最佳计划。
...
3. 执行以下命令以清除 AdventureWorks2017 数据库的计划缓存
...
GO
```
4. 现在，使用查询提示运行查询。查看计划，请注意，即使@SalesPersonID = 277，它现在也使用计划为值288创建的索引查找。
