...
2. 修复键查找，并重新运行查询以查看新计划。通过添加包含所有查询返回或搜索字段的 COVERING 索引来修复键查找。在本示例中，该索引只有 ProductID。如果将“输出列表”字段作为“包含的列”添加到索引，则会删除“键查找”。由于已存在该索引，因此必须删除索引并重新创建索引，或者设置 DROP_EXISTING = ON 方可添加列。备注：ProductID 已经是索引的一部分，无需作为包含的列进行添加。
